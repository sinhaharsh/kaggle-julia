...
a_1 = [ones(m, 1) X];
z_2 = a_1 * Theta1';
a_2 = sigmoid(z_2);%5000*401 * 401*25 
a_2 = [ones(m, 1) a_2];%5000*26
z_3 = a_2 * Theta2';
h = sigmoid(z_3);%5000*26 * 26*10
mInv = power(m, -1);
...
reguConst = ((0.5 * lambda * mInv) * (sum(sum(temp_1 .^ 2)) + sum(sum(temp_2 .^ 2))));
%y - 5000*1
for k = 1:num_labels
    yk = y == k;
    hk = h(:, k);
...
for t = 1:m
    ...
    D2 = delta3 * a2;%10*1 * 1*26 
    D1 = delta2 * a1;%26*1 * 1*401
